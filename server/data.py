...
class EXAMINECSV():
    ...
    def get_average_game_ratings_by_genre_and_year(self, genre): 
        ...
        if average_ratings.empty:
            # Handle the case where no games are found for the selected year
            year_and_critic_ratings.append(["No games found for the selected year", 0])  # Default score of 0
        else: 
    ...
    def Top_sales_by_publisher_by_selected_year(self, year):
        ...
        if selected_year_df.empty:
            # Handle the case where no data is found for the selected year
            top_publishers_by_selected_year.append(["No data found for the selected year", 0])  # Default sales of 0
        else:
            ...
            for index, sales in top_publishers_sorted.iterrows():
                publisher = sales['Publisher']
                total_sales = int(sales['Global_Sales'])
                top_publishers_by_selected_year.append([publisher, total_sales])
    ...
    def top_sales_by_publisher_by_selected_year_and_genre(self, year, genre):
        ...
        if selected_data.empty:
            # Handle the case where no data is found for the selected year and genre
            top_publishers_by_selected_year_and_genre.append(["No data found for the selected year and genre", 0])  # Default sales of 0
        else:
